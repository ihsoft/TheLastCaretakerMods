...
# build 24990438, VoyageSteam-Win64-Shipping.exe SHA-256
...
$expectedSteamBuildId = '24990438'
$expectedExecutableSha256 = 'D9BF4C9624C60615198E62C87DA7792A9888AB02F7905AAAF1C9B02C7A9E524F'
...
$replacementPackage = '/Game/Mods/BoatTotalDiesel/WBP_BoatTotalDieselValue'
$assetName = 'BP_VoyageIngameBoatHud'
$relocatedAssetName = 'BP_VoyageIngameBoatHud_O'
$replacementAssetName = 'WBP_BoatTotalDieselValue'
$initialText = '-- L'
...
if ([string]$fingerprint.steam.buildId -cne $expectedSteamBuildId -or
    ...
    throw 'Installed game fingerprint does not match the BoatTotalDiesel contracts.'
}
...
if (Test-Path -LiteralPath $generatedContent) {
    ...
    if (-not $resolvedGeneratedContent.StartsWith($projectRoot + [IO.Path]::DirectorySeparatorChar)) {
        throw "Refusing to clean Content outside the BoatTotalDiesel project: $resolvedGeneratedContent"
    }
...
if ($generateProcess.ExitCode -ne 0) {
    throw "BoatTotalDiesel generation failed with exit code $($generateProcess.ExitCode)."
}
...
$cookedReplacementLive = Join-Path $cookedContent 'Mods\BoatTotalDiesel\WBP_BoatTotalDieselValue'
foreach ($cookedAssetBase in @($cookedBase, $cookedReplacementLive)) {
...
function Invoke-ModCook {
    ...
    if ($cookProcess.ExitCode -ne 0) {
        throw "BoatTotalDiesel cook failed for $PackageName with exit code $($cookProcess.ExitCode)."
    }
}

Invoke-ModCook -PackageName $replacementPackage -LogName 'cook-replacement.log'
foreach ($extension in @('.uasset', '.uexp')) {
    if (-not (Test-Path -LiteralPath ($cookedReplacementLive + $extension) -PathType Leaf)) {
        throw "Expected cooked replacement file was not produced: $cookedReplacementLive$extension"
    }
}
$replacementSnapshotDirectory = Join-Path $output 'cooked-replacement'
[IO.Directory]::CreateDirectory($replacementSnapshotDirectory) | Out-Null
$cookedReplacement = Join-Path $replacementSnapshotDirectory $replacementAssetName
Copy-Item -LiteralPath `
    ($cookedReplacementLive + '.uasset'), ($cookedReplacementLive + '.uexp') `
    -Destination $replacementSnapshotDirectory
Invoke-ModCook -PackageName $sourcePackage -LogName 'cook-child.log'

foreach ($cookedAssetBase in @($cookedBase, $cookedReplacement)) {
    foreach ($extension in @('.uasset', '.uexp')) {
...
if (-not $cookedChildNameTable.Contains('PreConstruct') -or
    -not $cookedChildNameTable.Contains($replacementAssetName) -or
...
$replacementNameTable = [Text.Encoding]::ASCII.GetString(
    [IO.File]::ReadAllBytes($cookedReplacement + '.uasset'))
if (-not $replacementNameTable.Contains('DieselValueText')) {
    throw 'Cooked replacement widget does not contain its expected text leaf.'
}
foreach ($requiredName in @('Tick', 'GetModulesInSameGrid', 'GetResourceAmount', 'SetText', 'Diesel')) {
...
$replacementStage = Join-Path $stage 'Voyage\Content\Mods\BoatTotalDiesel'
...
Copy-Item -LiteralPath `
    ($cookedReplacement + '.uasset'), ($cookedReplacement + '.uexp') `
...
$expectedAssetPaths = @(
    ...
    '../../../Voyage/Content/Mods/BoatTotalDiesel/WBP_BoatTotalDieselValue.uasset'
...
if ($inventoryRecords.Count -ne $expectedAssetPaths.Count -or $inventoryDifference.Count -ne 0) {
    throw 'Packaged asset inventory does not match the exact three-asset mod set.'
}
...
Write-Host 'BoatTotalDiesel mod built successfully.'
Get-FileHash -Algorithm SHA256 -LiteralPath $containerFiles |
