...
if ($schema.schemaVersion -ne 1) { throw 'Unsupported StableGyro settings schema version.' }
if ($null -eq $schema.section -or [string]$schema.section -cne '') {
    throw 'StableGyro uses a flat INI; schema section must be an empty string.'
}
$schemaSettings = @($schema.settings)
if ($schemaSettings.Count -eq 0) { throw 'StableGyro settings schema has no settings.' }
...
if ($iniValues.Count -eq 0) { throw 'StableGyro default INI has no settings.' }
...
$header.Add('// Generated by Build/New-StableGyroSettings.ps1. Do not edit.')
