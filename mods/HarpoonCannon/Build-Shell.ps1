...
Invoke-NativeStage 'cook' $editor @($project,'-run=cook','-targetplatform=Windows','-unversioned','-SkipZenStore','-CookSinglePackageNoRefs',('-Package=' + ($packages -join '+')),'-unattended','-nop4','-nosplash','-nullrhi',('-abslog=' + (Join-Path $output 'cook-unreal.log')))
...
$provenance = [ordered]@{
    ...
    validation='static-only; runtime pending';runtimeArchitecture='Cyclone leaf VoyageModuleActor shell and three rough meshes; no operator, Drone, HUD, autoload or event graph';
