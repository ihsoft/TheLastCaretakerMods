...
Require-Child $entryConfig.node 'HarpoonEntryReference'
$entry = @($shell | Where-Object { $_.Name -ceq 'HarpoonEntryReference_GEN_VARIABLE' })
Require ($entry.Count -eq 1 -and $entry[0].Type -ceq 'BoxComponent') 'Missing entry reference box.'
Require (@($entry[0].Properties.ComponentTags) -ccontains 'Harpoon.Model.Entry') 'Entry reference tag missing.'
