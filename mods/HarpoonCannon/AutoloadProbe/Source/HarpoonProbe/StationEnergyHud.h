...
// Diagnostic values come from the cannon module, not the possessed station.
...
void UpdateStationEnergyHud(FGraph& G, UEdGraphPin* Station, UClass* StationClass)
{
    ...
    auto* Cannon = ObserveCall(G, UActorComponent::StaticClass(), OP::ComponentOwner, Anchor);
    G.Branch(G.Valid(Cannon));
    auto* Find = G.Call(AActor::StaticClass(), OpticalCameraGraphNames::FindComponent);
    G.Link(Cannon, G.Pin(Find, P::FunctionTarget));
