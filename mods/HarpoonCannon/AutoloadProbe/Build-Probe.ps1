...
if (-not $OutputRoot) { $OutputRoot = Join-Path $repo ('artifacts/harpoon-cannon/hc02-' + [DateTime]::UtcNow.ToString('yyyyMMdd-HHmmss')) }
...
$provenance = [ordered]@{
    schemaVersion=1;mod='HarpoonCannonLifecycleProbe';version=$version;experiment='HC02';createdAtUtc=[DateTime]::UtcNow.ToString('o');
