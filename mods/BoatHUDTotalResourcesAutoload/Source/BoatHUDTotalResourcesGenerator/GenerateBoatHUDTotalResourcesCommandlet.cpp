...
namespace
{
constexpr TCHAR ChildPackageName[] = TEXT("/Game/Mods/BoatHUDTotalResources/BP_BoatHUDTotalResources");
constexpr TCHAR TemporaryParentPackageName[] = TEXT("/Game/Mods/Boat/BP_VoyageIngameBoatHud_O");
...
constexpr TCHAR SwapActorPackageName[] = TEXT("/Game/Mods/BoatHUDTotalResources/ModActor");
constexpr TCHAR SwapActorAssetName[] = TEXT("ModActor");
constexpr TCHAR ChildAssetName[] = TEXT("BP_BoatHUDTotalResources");
constexpr TCHAR TemporaryParentAssetName[] = TEXT("BP_VoyageIngameBoatHud");
...
const FName TemporaryParentBlueprintName(TEXT("GenerateBoatHUDTotalResourcesParent"));
const FName SwapActorBlueprintName(TEXT("GenerateBoatHUDTotalResourcesSwapActor"));
const FName EventGraphName(TEXT("EventGraph"));
const FName VisibilityInputPinName(TEXT("InVisibility"));
const FName PlayerIndexInputPinName(TEXT("PlayerIndex"));
const FName WorldContextInputPinName(TEXT("WorldContextObject"));
const FName WidgetTypeInputPinName(TEXT("WidgetType"));
const FName WidgetClassInputPinName(TEXT("WidgetClass"));
const FName TopLevelOnlyInputPinName(TEXT("TopLevelOnly"));
const FName TargetMapInputPinName(TEXT("TargetMap"));
const FName MapKeyInputPinName(TEXT("Key"));
const FName MapValueInputPinName(TEXT("Value"));
...
const FName GetHUDFunctionName(TEXT("GetHUD"));
...
UEdGraphPin* FindFirstOutputArrayPin(UEdGraphNode* Node)
{
    for (UEdGraphPin* Pin : Node->Pins)
    {
        if (Pin->Direction == EGPD_Output && Pin->PinType.ContainerType == EPinContainerType::Array)
        {
            return Pin;
        }
    }
...
bool AddHUDSwapActorGraph(
    UBlueprint* Blueprint,
    UClass* ReplacementWidgetClass)
{
    check(Blueprint && ReplacementWidgetClass);
    ...
    UK2Node_Event* BeginPlayEvent = NewObject<UK2Node_Event>(Graph);
    BeginPlayEvent->EventReference.SetExternalMember(
        FName(TEXT("ReceiveBeginPlay")),
        AActor::StaticClass());
    BeginPlayEvent->bOverrideFunction = true;
    FinishNode(BeginPlayEvent, Graph, -620, 0);

    UFunction* GetPlayerControllerFunction = UGameplayStatics::StaticClass()->FindFunctionByName(
        GET_FUNCTION_NAME_CHECKED(UGameplayStatics, GetPlayerController));
    UFunction* GetHUDFunction = APlayerController::StaticClass()->FindFunctionByName(
        GetHUDFunctionName);
    UFunction* MapAddFunction = UBlueprintMapLibrary::StaticClass()->FindFunctionByName(
        GET_FUNCTION_NAME_CHECKED(UBlueprintMapLibrary, Map_Add));
    if (!GetPlayerControllerFunction || !GetHUDFunction || !MapAddFunction)
    {
        UE_LOG(LogTemp, Error, TEXT("One or more required HUD-class replacement functions were not found"));
        return false;
    }

    const UEdGraphSchema_K2* Schema = GetDefault<UEdGraphSchema_K2>();
    UK2Node_CallFunction* GetPlayerController = AddCall(
        Graph, GetPlayerControllerFunction, -360, 200);
    RequirePin(GetPlayerController, PlayerIndexInputPinName)->DefaultValue = TEXT("0");

    UK2Node_CallFunction* GetHUD = AddCall(Graph, GetHUDFunction, -100, 160);

    UK2Node_DynamicCast* CastHUD = NewObject<UK2Node_DynamicCast>(Graph);
    CastHUD->TargetType = AVoyageHUD::StaticClass();
    FinishNode(CastHUD, Graph, 160, 0);
    CastHUD->SetPurity(false);

    UK2Node_VariableGet* GetCharacterWidgetClasses = NewObject<UK2Node_VariableGet>(Graph);
    GetCharacterWidgetClasses->VariableReference.SetExternalMember(
        GET_MEMBER_NAME_CHECKED(AVoyageHUD, CharacterWidgetClasses),
        AVoyageHUD::StaticClass());
    FinishNode(GetCharacterWidgetClasses, Graph, 420, 220);

    UK2Node_CallFunction* AddBoatHUDClass = AddCall(Graph, MapAddFunction, 680, 0);

    UK2Node_Self* SelfNode = NewObject<UK2Node_Self>(Graph);
    FinishNode(SelfNode, Graph, -620, 220);
    UEdGraphPin* SelfPin = RequirePin(SelfNode, UEdGraphSchema_K2::PN_Self);

    const bool Connected =
        Connect(Schema, RequirePin(BeginPlayEvent, UEdGraphSchema_K2::PN_Then),
            RequirePin(CastHUD, UEdGraphSchema_K2::PN_Execute)) &&
        Connect(Schema,
            SelfPin,
            RequirePin(GetPlayerController, WorldContextInputPinName)) &&
        Connect(Schema,
            RequirePin(GetPlayerController, UEdGraphSchema_K2::PN_ReturnValue),
            RequirePin(GetHUD, UEdGraphSchema_K2::PN_Self)) &&
        Connect(Schema,
            RequirePin(GetHUD, UEdGraphSchema_K2::PN_ReturnValue),
            CastHUD->GetCastSourcePin()) &&
        Connect(Schema,
            CastHUD->GetValidCastPin(),
            RequirePin(AddBoatHUDClass, UEdGraphSchema_K2::PN_Execute)) &&
        Connect(Schema,
            CastHUD->GetCastResultPin(),
            RequirePin(GetCharacterWidgetClasses, UEdGraphSchema_K2::PN_Self)) &&
        Connect(Schema,
            GetCharacterWidgetClasses->GetValuePin(),
            RequirePin(AddBoatHUDClass, TargetMapInputPinName));
    if (!Connected)
    {
        UE_LOG(LogTemp, Error, TEXT("Failed to connect swap actor BeginPlay graph"));
        return false;
    }

    RequirePin(AddBoatHUDClass, MapKeyInputPinName)->DefaultObject = AVoyageBoatPawn::StaticClass();
    RequirePin(AddBoatHUDClass, MapValueInputPinName)->DefaultObject = ReplacementWidgetClass;
...
{
    if (FPackageName::DoesPackageExist(TemporaryParentPackageName) ||
        FPackageName::DoesPackageExist(ChildPackageName) ||
        FPackageName::DoesPackageExist(SwapActorPackageName) ||
    ...
    UPackage* TemporaryParentPackage = CreatePackage(TemporaryParentPackageName);
    UWidgetBlueprint* TemporaryParentBlueprint = Cast<UWidgetBlueprint>(
        FKismetEditorUtilities::CreateBlueprint(
            UVoyageInGameBoatWidget::StaticClass(),
            TemporaryParentPackage,
            FName(TemporaryParentAssetName),
            BPTYPE_Normal,
            UWidgetBlueprint::StaticClass(),
            UWidgetBlueprintGeneratedClass::StaticClass(),
            TemporaryParentBlueprintName));
    if (!TemporaryParentBlueprint || !SaveBlueprint(TemporaryParentPackage, TemporaryParentBlueprint))
    {
    ...
    UWidgetBlueprint* ChildBlueprint = Cast<UWidgetBlueprint>(
        FKismetEditorUtilities::CreateBlueprint(
            TemporaryParentBlueprint->GeneratedClass,
            ChildPackage,
            FName(ChildAssetName),
            BPTYPE_Normal,
            UWidgetBlueprint::StaticClass(),
            UWidgetBlueprintGeneratedClass::StaticClass(),
            ChildBlueprintName));
    ...
    UPackage* SwapActorPackage = CreatePackage(SwapActorPackageName);
    UBlueprint* SwapActorBlueprint = Cast<UBlueprint>(
        FKismetEditorUtilities::CreateBlueprint(
            AActor::StaticClass(),
            SwapActorPackage,
            FName(SwapActorAssetName),
            BPTYPE_Normal,
            UBlueprint::StaticClass(),
            UBlueprintGeneratedClass::StaticClass(),
            SwapActorBlueprintName));
    if (!SwapActorBlueprint ||
        !AddHUDSwapActorGraph(
            SwapActorBlueprint,
            ChildBlueprint->GeneratedClass) ||
        !SaveBlueprint(SwapActorPackage, SwapActorBlueprint))
    {
        UE_LOG(LogTemp, Error, TEXT("Failed to create the Boat HUD swap actor"));
        return 1;
    }

    UE_LOG(LogTemp, Display, TEXT("Generated temporary parent: %s"),
        TemporaryParentPackageName);
    UE_LOG(LogTemp, Display, TEXT("Generated child override: %s.%s"),
        ChildPackageName, ChildAssetName);
    UE_LOG(LogTemp, Display, TEXT("Generated swap actor: %s.%s"),
        SwapActorPackageName, SwapActorAssetName);
