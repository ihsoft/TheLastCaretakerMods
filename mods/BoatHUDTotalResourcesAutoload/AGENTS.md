...
These rules apply to the autoload/DML-compatible BoatHUDTotalResources mod in
this directory. Its filesystem project name is `BoatHUDTotalResourcesAutoload`,
while all runtime mod, package, and container identities remain
...
- The native `VoyageHUD.CharacterWidgetClasses` replacement, the physical
  container identity without `_P`, and the DML-compatible entry Blueprint
  `/Game/Mods/BoatHUDTotalResources/ModActor.ModActor_C` passed a real-game load
  check on Steam build `25056839`.
