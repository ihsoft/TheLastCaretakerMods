...
foreach ($record in @($manifest.retiredLegacyFiles)) {
    $backup = (Resolve-Path -LiteralPath ([string]$record.backupPath)).Path
    if ((Get-FileHash -Algorithm SHA256 -LiteralPath $backup).Hash -cne [string]$record.sha256) {
        throw "Retired legacy-container backup hash mismatch: $backup"
    }
    $restoreTarget = Join-Path $paks ([string]$record.name)
    if (Test-Path -LiteralPath $restoreTarget) {
        throw "Refusing to overwrite a legacy container file: $restoreTarget"
    }
}
...
foreach ($record in @($manifest.retiredLegacyFiles)) {
    Copy-Item -LiteralPath ([string]$record.backupPath) -Destination (
        Join-Path $paks ([string]$record.name))
}
