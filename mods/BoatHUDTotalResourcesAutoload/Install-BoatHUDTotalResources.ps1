...
if ([string]$buildManifest.kind -cne 'BoatHUDTotalResources mod' -or
    [string]$buildManifest.mountEvent -cne
        'AActor::BeginPlay updates VoyageHUD.CharacterWidgetClasses' -or
    [string]$buildManifest.stockPetrolVisibility -cne 'Collapsed' -or
    [string]$buildManifest.stockBatteryVisibility -cne 'Collapsed' -or
    [string]$buildManifest.childBlueprintPackage -cne '/Game/Mods/BoatHUDTotalResources/BP_BoatHUDTotalResources' -or
    [string]$buildManifest.childBlueprintAsset -cne 'BP_BoatHUDTotalResources' -or
    [string]$buildManifest.swapActorBlueprintPackage -cne
        '/Game/Mods/BoatHUDTotalResources/ModActor' -or
    [string]$buildManifest.swapActorAsset -cne 'ModActor' -or
    ...
    [string]$buildManifest.executableSha256 -cne
        'CA84428CF4562C703BEDFF053DB727D14CC70C593451C09BE75A92828EFD9933' -or
    [string]$buildManifest.autoloadFile -cne ($ContainerName + '.autoload') -or
    [string]$buildManifest.autoloadEntryClass -cne
        '/Game/Mods/BoatHUDTotalResources/ModActor.ModActor_C') {
...
$legacyContainerName = 'BoatHUDTotalResources_P'
$legacyBackup = Join-Path $evidence 'retired-legacy-container'

$extensions = @('.pak', '.ucas', '.utoc', '.autoload')
$records = @()
foreach ($extension in $extensions) {
    ...
    if ($extension -ceq '.autoload') {
        $expectedAutoload = 'entryClass: ' + [string]$buildManifest.autoloadEntryClass + [Environment]::NewLine
        if ([IO.File]::ReadAllText($source) -cne $expectedAutoload) {
            throw "BoatHUDTotalResources autoload content is invalid: $source"
        }
    } else {
...
$legacyRecords = @()
foreach ($extension in $extensions) {
    $legacyName = $legacyContainerName + $extension
    $legacyPath = Join-Path $paks $legacyName
    if (-not (Test-Path -LiteralPath $legacyPath -PathType Leaf)) {
        continue
    }
    [IO.Directory]::CreateDirectory($legacyBackup) | Out-Null
    $legacyHash = (Get-FileHash -Algorithm SHA256 -LiteralPath $legacyPath).Hash
    $legacyBackupPath = Join-Path $legacyBackup $legacyName
    Copy-Item -LiteralPath $legacyPath -Destination $legacyBackupPath
    if ((Get-FileHash -Algorithm SHA256 -LiteralPath $legacyBackupPath).Hash -cne $legacyHash) {
        throw "Legacy BoatHUDTotalResources backup hash mismatch: $legacyBackupPath"
    }
    $legacyRecords += [pscustomobject][ordered]@{
        name = $legacyName
        sha256 = $legacyHash
        backupPath = $legacyBackupPath
    }
}
foreach ($legacyRecord in $legacyRecords) {
    Remove-Item -LiteralPath (Join-Path $paks ([string]$legacyRecord.name)) -Force
}
...
$manifest = [ordered]@{
    ...
    retiredLegacyContainerName = $legacyContainerName
    installedAtUtc = [DateTime]::UtcNow.ToString('o')
    files = $records
    retiredLegacyFiles = $legacyRecords
}
