# Historical model work — superseded by GLB V1

This is an archive, not current authoring/build guidance. See the current model
backlog and models/HarpoonCannon/model-source.json. Paths/commands below describe
past checkpoints, some tools have moved; do not run them to regenerate V1.

# HarpoonCannon model backlog

## Restart / current state

- Electrical socket reference completed: user accepted approximate colors for
  planning ONLY; final cannon will have an anchor for the original game socket.
  Output `artifacts/modeling/ElectricalSocket/glb/ElectricalSocket_Reference.glb`,
  SHA256 `53C5B34466EC1BB75C0B63ED8CE90524EC00497A408B6298C9C1DD24E02DD735`.
  Parent folder contains source JSON, blend, audits and `socket-front.png`.
  Original LOD0 3504 vertices/2932 triangles/3 slots; one duplicate-index face
  split into 3 coincident vertices for Blender (3507 vertices), all triangles,
  positions, UVs and normals retained. Pivot retained; UE cm -> Blender meters
  (X,-Y,Z); socket-data Roll90 NOT applied. Bounds ~25.46 x15.92 x25.45 cm.
  Approximate metal/blue slots, no textures or glyph. Blend reopen error=0,
  GLB re-import max world error=2.1073424e-8 meters. Front render inspected.
  Method retained in tools/VoyageMeshReference; outputs ignored, no commit.
  No Harpoon geometry/descriptor/gameplay/build/install changes.
- Extraction report: public fingerprint/mapping succeeded on same build below;
  stock-only parser and bundle hash checks passed. New geometry reader was needed
  because JSON tools omit raw vertex/index data. SDK NuGet-config denial required
  escalated development publish; CUE API adjusted to LoadPackage/GetExports.
  Blender validation caught source duplicate face, resolved without geometry loss.
  Existing GLB converter reused; sibling audit collision avoided with GLB subfolder.
  No texture decoder or broader model pipeline developed. Gap recorded in toolchain
  backlog; source-only Blender work excluded from game-tool coverage.

- Current requested game-asset export: user narrowed Typhoon GLB to ONLY its
  electrical socket, not the turbine assembly. Read-only identification on Steam
  25191271 / EXE747DC2553F7E68D8EA7ED0B2E0CAC6D08943EA3F50DD6ED822E9293E0B45F58B
  confirms DA_Item_Module_WindTurbine Name=Typhoon and its Large Blueprint.
  That Blueprint's socket references `/Game/Data/Assets/ModuleSockets/DA_Socket_ElectricData`.
  MeshIn and MeshOut both use `/Game/AssetSets/Sockets/SM_Socket_Electrical_Output`;
  socket-data Roll=90 degrees. Three identical in/out material overrides:
  MI_Socket_Metal_Trim, MI_Socket_ResourceDetail_Electric, MI_Socket_NoEmis.
  Their parents include layered/object/world-space dirt and a parameterized
  masked symbol shader. A full Unreal shader cannot be treated as ordinary GLB
  texture assignment; user then accepted approximate flat colors for planning,
  delivered above. Full material export remains out of scope.
  Public fingerprint/mapping/list/JSON/summary calls succeeded; mappings match
  E25620FB2D0674B26A3E4EF059AC2C5AF58E28CE1B4F07307860F7FADB9A0F68.
  Tool gap: current public routes inspect assets, not textured-model export.
  Bounded source check (trigger: no documented conversion interface) found the
  local CUE4Parse-Conversion GLTF writer creates white slot materials, not full
  material conversion. No tool build/install or source model mutation attempted.

- GLB delivery: `artifacts/modeling/HarpoonCannon/glb/HarpoonCannon_v5.glb`,
  SHA256 `2CF0FED562C2DD9B4DD3256EB9F8BF35CA7EAAB9140C5938FAAC150C521AE2D8`.
  `models/HarpoonCannon/export_glb.py` wraps the existing bundled Blender glTF
  exporter, not a custom converter. README records repeatable export and
  `--verify-only` commands. Preserves 88 nodes / 83 mesh objects / 5990 triangles /
  9 palette materials, hierarchy and separate ammo; excludes studio camera/lights.
  Standard Y-up/meter GLB is self-contained. Material values, including emission,
  match the blend. Separate re-import verification max world error
  `2.4027401e-7` meters; source blend SHA unchanged. No gameplay/build/install or
  source geometry/material/descriptor changes; no commit requested.
- GLB tool report: intended blend -> GLB via installed Blender CLI/io_scene_gltf2;
  exporter property probe, export and separate verify-only all passed. No
  converter dependency download/install or third-party implementation inspection.
  Online Blender manual fetch returned HTTP 402; installed operator metadata was
  sufficient. Old importer labels its successful progress messages CRITICAL;
  operation returned FINISHED and geometry checks passed. Initial exporter output
  was verbose; no repeat/debug investigation needed. Source modeling coverage N/A;
  no reusable game-tool gap. Local script and docs retained, generated file ignored.

- Blender delivery: `artifacts/modeling/HarpoonCannon/blender/HarpoonCannon_v5.blend`
  (1,478,712 bytes), SHA256 `F56BDB39F213667FE828BA24A53657537333136F431CF31F9420F1C69A2D2EBE`.
  User requested the file and a reusable script; `models/HarpoonCannon/export_blender.py`
  implements export, re-open audit, material/geometry checks and unsaved articulation
  test. See model README for command and Blender controls. No commit requested.
- Blender QA: installed 2.80 sub39, 83 named mesh objects / 5990 triangles / 9
  materials; six independent ammo objects share one mesh. Coordinates converted
  cm -> meters, pivots/HC_Sight retained. Reopened world error <=0.000018 cm;
  yaw37/pitchY-8 articulation error <=0.000027 cm. Direct Cycles render inspected.
  No game source/descriptor/OBJ/material-palette/build/install changes.
- Export tool report: existing Blender CLI + bpy only; no installation/download.
  Early 2.80 compatibility fixes were needed (RGB viewport color, no Principled
  emission socket/backface flag, legacy OCIO name, missing __file__ in --python).
  Initial thumbnail-directory warnings led to one outside-sandbox retry, which
  did not resolve the script issue; runpy exposed the actual API failures. Final
  normal --python entry point and separate --verify-only invocation passed.
  Source-only workflow: game-tool coverage N/A, no reusable game-tool gap.
