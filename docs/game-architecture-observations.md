...
- Простое создание контейнера через `WidgetBlueprintLibrary:Create` возможно.
- Попытка вызвать `SetActions` с одним массивом неверна: реальная функция ожидает
  четыре параметра.
- `SetInputs` использует карты с локализованными `FText`, а не простой массив
  действий.
- Создание нового `FText` из Lua в этой сборке UE4SS Compact ранее приводило к
  ошибке `FText(FString&&) not found` и падению.
- Поэтому штатные подсказки `X/C` пока не созданы. Безопаснее либо разобраться с
  полной сигнатурой и готовыми игровыми `FText`, либо рисовать короткие метки в
  независимом пользовательском HUD.
